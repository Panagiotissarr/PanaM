...
PanaM's ESP cheats are completely client-side, and thus undetectable by anticheat
...
PanaM's Sabotage cheats work even if you aren't impostor and are subject to no cooldown.
...
You can change all of the following settings in `BepInEx/config/PanaM.cfg`
...
| GUI.Color | Sets the color of PanaM's GUI using HTML color codes | String | |
...
## Other relevant features of PanaM:

- PanaM has a simple **GUI** that is easy to navigate and can be toggled using the **DELETE** key on your keyboard
- [**TEMPORARILY BROKEN**] PanaM comes with **custom announcements** that it will automatically fetch online at launch.
