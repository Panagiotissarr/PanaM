...
public struct CheatToggles
{
    ...
    // Saves cheat toggles and their keybinds to PanaMProfile.txt
    // Format per line: ToggleName = True/False = KeyCode.KEY
    public static void SaveTogglesToProfile()
    {
        using var writer = new StreamWriter(PanaM.ProfilePath);

        writer.WriteLine("# PanaMProfile");
    ...
    // Loads cheat toggles and their keybinds from PanaMProfile.txt if the file is present
    // Format per line: ToggleName = True/False = KeyCode.KEY
    public static void LoadTogglesFromProfile()
    {
        if (!File.Exists(PanaM.ProfilePath)) return;

        using var reader = new StreamReader(PanaM.ProfilePath);
