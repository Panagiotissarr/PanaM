...
public class OverloadUI : MonoBehaviour
{
    ...
    private void Start()
    {
        killSwitchThreshold = 500 * PanaM.killSwitchLvl.Value;

        if (!CheatToggles.olAutoAdapt)
        {
            OverloadHandler.strength = PanaM.defaultStrength.Value;
            OverloadHandler.cooldown = PanaM.defaultCooldown.Value;
        }
    }
