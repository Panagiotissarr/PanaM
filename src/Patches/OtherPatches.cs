...
public static class Constants_GetPlatformData
{
    ...
    {
        if (Utils.StringToPlatformType(PanaM.spoofPlatform.Value, out Platforms? platformType))
        {
...
public static class ChatBubble_SetName
{
	...
	{
        PanaMESP.ChatNametags(__instance);
    }
}
...
public static class SystemInfo_deviceUniqueIdentifier_Getter
{
    ...
    {
        if (!PanaM.spoofDeviceId.Value) return;
...
{
    // Postfix patch of VersionShower.Start to show PanaM version
    public static void Postfix(VersionShower __instance)
    {
        if (PanaM.inStealthMode || PanaM.isPanicked) return;

        if (PanaM.supportedAU.Contains(Application.version)) // Checks if Among Us version is supported
        {
            __instance.text.text =  $"PanaM v{PanaM.panamVersion} (v{Application.version})"; // Supported
        }
        else
        {
            __instance.text.text =  $"PanaM v{PanaM.panamVersion} (<color=red>v{Application.version}</color>)"; // Unsupported
        }
    }
}
...
{
    // Postfix patch of PingTracker.Update to show PanaM authors and colored ping text
    public static void Postfix(PingTracker __instance)
    {
        if (PanaM.inStealthMode)
        {
        ...
        if (AmongUsClient.Instance.IsGameStarted)
        {
            ...
            __instance.text.text = $"PanaM by scp222thj & Astral ~ {pingText}";

            return;
        }

        __instance.text.text = $"PanaM by scp222thj & Astral\n{pingText}";
...
public static class Mushroom_FixedUpdate
{
    ...
    {
        PanaMESP.SporeCloudVision(__instance);
    }
}
